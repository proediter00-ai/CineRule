...
object PluginManager {
    ...
    suspend fun ___DO_NOT_CALL_FROM_A_PLUGIN_loadAllLocalPlugins(context: Context, forceReload: Boolean) {
        ...
        // Use app-specific external files directory for loading plugins.
        ...
        // --- CineRule: Load bundled plugins from internal storage (no permissions needed) ---
        try {
            val bundledDir = File(context.filesDir, "bundled_plugins")
            if (bundledDir.exists()) {
                val bundledPlugins = bundledDir.listFiles()?.filter {
                    it.extension == "cs3" || it.extension == "zip"
                }
                Log.d(TAG, "Bundled plugins found: ${bundledPlugins?.size}")
                bundledPlugins?.forEach { file ->
                    try {
                        ...
                        if (!destinationFile.exists() ||
                            ...
                            Log.i(TAG, "Copied bundled plugin: ${file.name}")
                        }
                        maybeLoadPlugin(context, destinationFile)
                    } catch (t: Throwable) {
                        Log.e(TAG, "Failed to load bundled plugin: ${file.name}")
                        logError(t)
                    }
                }
            }
        } catch (t: Throwable) {
            Log.e(TAG, "Failed to process bundled plugins")
            logError(t)
        }
        // --- End CineRule bundled plugins ---

        // Also try to load from LOCAL_PLUGINS_PATH (external storage) if accessible
        ...
        try {
            ...
            Log.w(TAG, "Could not access LOCAL_PLUGINS_PATH, skipping external storage plugins")
            logError(t)
        }
